"""
Train Heart Disease Model with Real Dataset
Uses the actual heart.csv file
"""
...
def train_heart_with_real_data(dataset_path: str = "heart.csv"):
    """
    Train heart disease model using the real dataset

    Args:
        dataset_path: Path to the CSV file
    """
    logger.info("=" * 60)
    logger.info("Training Heart Disease Model with Real Dataset")
    logger.info("=" * 60)
    ...
    logger.info(f"\nTarget distribution:\n{target_dist}")
    logger.info(f"Heart disease rate: {target_dist[1] / len(df) * 100:.2f}%")
    ...
    model_data = {
        ...
        "dataset_info": {
            ...
            "heart_disease_rate": float(target_dist[1] / len(df)),
        },
    }
    ...
    logger.info("=" * 60)
    print(classification_report(y_test, ensemble_pred, target_names=["No Heart Disease", "Heart Disease"]))
...
if __name__ == "__main__":
    dataset_path = sys.argv[1] if len(sys.argv) > 1 else "heart.csv"

    if not os.path.exists(dataset_path):
        print(f"❌ Error: Dataset file not found: {dataset_path}")
        sys.exit(1)

    result = train_heart_with_real_data(dataset_path)
    print("\nModel training completed successfully!")
    print(f"Best ensemble accuracy: {result['results']['ensemble']['accuracy']:.4f}")
